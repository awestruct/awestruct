  ...
  class Engine
    ...
    def generate_page(page, generated_path, produce_output=true)
      if ( produce_output )
        ...
        if page.dependencies.load!
          $LOG.info "Cached:     #{generated_path}" if $LOG.info?
        else
          $LOG.info "Analyzing:  #{generated_path}" if $LOG.info?
    ...
    def generate_page_by_output_path(path)
      ...
      pages.each{|page|
        ...
        $LOG.info "Page: #{page.output_path} #{page.relative_source_path} #{page.__is_layout ? 'Layout':''}" if $LOG.info?
        $LOG.info "Detected change in content (#{page.dependencies.content_hash})" if page.dependencies.has_changed_content if $LOG.info?
        $LOG.info "!! Detected change in front matter. To fully reflect the change you'll need to restart Awestruct (#{page.dependencies.key_hash})" if page.dependencies.has_changed_keys if $LOG.info?
        $LOG.info "No changes detected" unless page.dependencies.has_changed_content or page.dependencies.has_changed_keys if $LOG.info?
        $LOG.info "Dependencies Matrix: (non unique source path)" if $LOG.info?
        $LOG.info "\t Outgoing dependencies:" if $LOG.info?
        $LOG.info "\t\t Content -> #{page.dependencies.dependencies.size}" if $LOG.info?
        $LOG.info "\t\t Key     -> #{page.dependencies.key_dependencies.size}" if $LOG.info?
        $LOG.info "\t Incoming dependencies:" if $LOG.info?
        $LOG.info "\t\t Content <- #{page.dependencies.dependents.size}" if $LOG.info?
        $LOG.info "\t\t Key     <- #{page.dependencies.key_dependents.size}" if $LOG.info?
        $LOG.info "--------------------" if $LOG.info?
      }
      ...
      $LOG.info "Starting regeneration of content dependent pages:" if regen_pages.size > 0 && $LOG.info?
